...
class _APODHomePageState extends State<APODHomePage> {
  ...
  Future<void> _setWallpaper(String imageUrl) async {
    ...
    final wallpaperDir = Directory('${appDocDir.path}/NASAaPOD');
    if (!wallpaperDir.existsSync()) {
      wallpaperDir.createSync();
    }

    final fileName = _selectedDate.toString().split(' ')[0] + '.jpg';
        ...
        children: [
          CarouselSlider.builder(
            ...
            options: CarouselOptions(
              ...
              initialPage: _apodImageUrls.length - 1,
                  ...
                  ElevatedButton(
                    onPressed: () {
                      if (_apodImageUrls.isNotEmpty) {
                        _setWallpaper(_apodImageUrls[_apodImageUrls.length -
                            1]); // Set wallpaper with the currently displayed image
                      }
                    },
            ...
            onPressed: () {
              showDatePicker(
                ...
                if (selectedDate != null) {
                  ...
                  _fetchAPOD(_selectedDate);
                  for (int i = 1; i < _apodDates.length; i++) {
                    DateTime date = _apodDates[i];
                    _fetchAPOD(date);
                  }
                }
              });
